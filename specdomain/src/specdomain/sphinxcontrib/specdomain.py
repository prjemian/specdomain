...
class SpecObject(ObjectDescription):
    ...
    def _resolve_module_name(self, signode, modname, name):
        ...
        env_modname = self.options.get(
            'module', self.env.temp_data.get('spec:module', 'spec'))
        if modname:
    ...
    def _handle_function_signature(self, sig, signode):
        ...
        stack = [signode[-1]]
        counters = [0, 0]
        for token in spec_paramlist_re.split(arglist):
            if token == '[':
    ...
    def add_target_and_index(self, name, sig, signode):
        if name not in self.state.document.ids:
            ...
            if self.objtype =='function':
                finv = self.env.domaindata['spec']['functions']
                ...
                for arity_index in range(first, last+1):
                    if fname in finv and arity_index in finv[fname]:
                        self.env.warn(
                            self.env.docname,
                            ('duplicate SPEC function description'
                             'of %s, ') % name +
                            'other instance in ' +
                ...
                oinv = self.env.domaindata['spec']['objects']
                if name in oinv:
                    self.env.warn(
                        self.env.docname,
                        'duplicate SPEC object description of %s, ' % name +
...
class SpecCurrentModule(Directive):
    """
    ...
    def run(self):
        ...
        if modname == 'None':
            env.temp_data['spec:module'] = None
        else:
            env.temp_data['spec:module'] = modname
        return []


class SpecModule(Directive):
    """
    ...
    def run(self):
        ...
        env.domaindata['spec']['modules'][modname] = \
...
class SpecVariable(Directive):
    pass

class SpecModuleIndex(Index):
    """
    Index subclass to provide the SPEC module index.
    """

    name = 'modindex'
    localname = l_('SPEC Module Index')
...
class SpecXRefRole(XRefRole):
    def process_link(self, env, refnode, has_explicit_title, title, target):
        refnode['spec:module'] = env.temp_data.get('spec:module')
